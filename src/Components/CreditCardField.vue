<template>
    <div
        ...
        :class="{
            ...
            [`is-focused-${focused}`]: focused,
        }">
        <form-group>
            <div class="credit-card-field-rows">
                <div
                    class="credit-card-field-row"
                    :class="{ 'has-error': currentErrors.number }">
                    <input-field
                        id="number"
                        ref="number"
                        ...
                        @valid.native="
                            () => onValid('number', showName ? 'name' : 'expMonth')
                        ">
                        <template #activity>
                                ...
                                <div v-else key="icons" class="credit-card-icons">
                                    <animate-css mode="out-in" name="flip" y>
                                        <div
                                            v-if="focused !== 'cvc'"
                                            key="front"
                                            class="credit-card-field-icon-card">
                                            <icon
                                                :icon="[icon ? 'fab' : 'far', icon || 'credit-card']"
                                                :data-brand="(type && type.niceType) || 'unknown'"
                ...
                <animate-css name="fade">
                    <div
                        v-if="showSecurityFields && showName"
                        class="credit-card-field-row"
                        :class="{ 'has-error': currentErrors.name }">
                        <input-field
                            id="name"
                            ref="name"
                            v-model="card.name"
                ...
                <animate-css name="fade">
                    <div
                        v-if="showSecurityFields"
                        class="credit-card-field-row d-flex"
                        :class="{
                            'has-error':
                                currentErrors.expMonth ||
                                currentErrors.expYear ||
                                currentErrors.cvc ||
                                currentErrors.zip,
                        }">
                        <select-field
...
export default {
  ...
  computed: {
    invalidFeedback () {
      return (
        this.error ||
        Object.entries(this.currentErrors)
    ...
    totalValidFields () {
      return Object.entries(this.validated).filter(
    ...
    hasErrors () {
      return (
        !!this.error ||
        !!Object.entries(this.currentErrors).filter(([key, value]) => !!value)
          .length
      )
    },
      ...
      handler (value, oldValue) {
        ...
        entries.forEach(([key, value]) => {
          this.$set(
            this.currentErrors,
            key,
            value === false ? this.currentErrors[key] || true : false
          )
        })
  ...
  methods: {
    ...
    updateExpiration () {
      if (this.card.expMonth && this.card.expYear) {
        this.$set(
          this.card,
          'expiration',
          `${this.card.expMonth} / ${this.card.expYear}`
        )
      }
    },
    ...
    onValid (current, next) {
      if (
        this.$refs[current] &&
        this.$refs[current].$el.querySelector(':focus')
      ) {
        if (
          this.$refs[next] &&
          this.$refs[next].$el.classList.contains('is-empty')
        ) {
    ...
    onCardTypeChange ({ types }) {
      ...
      this.$set(
        this.card,
        'numberFormatted',
        format(this.card.number, this.type)
      )
